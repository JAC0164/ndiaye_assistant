...
- INTERDICTION d'exposer les secrets d'environnement : Ne jamais préfixer les variables contenant des clés privées ou des tokens d'IA par NEXT*PUBLIC*. Tout token sensible (comme le token Gemini ou la clé Supabase de service) doit rester invisible pour le navigateur.
