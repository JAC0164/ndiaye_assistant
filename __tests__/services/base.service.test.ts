...
describe("BaseService", () => {
  ...
  describe("getAll", () => {
    ...
    it("should apply orderBy option", async () => {
      mock.setResult(records)
      await service.getAll("user-1", { orderBy: { column: "name", ascending: true } })
    ...
    it("should apply limit option", async () => {
      mock.setResult(records)
      await service.getAll("user-1", { limit: 10 })
    ...
    it("should apply offset with range using default limit when no limit given", async () => {
      mock.setResult(records)
      await service.getAll("user-1", { offset: 5 })
    ...
    it("should apply offset with custom limit in range", async () => {
      mock.setResult(records)
      await service.getAll("user-1", { offset: 5, limit: 20 })
    ...
    it("should throw on database error", async () => {
      ...
      await expect(service.getAll("user-1")).rejects.toThrow("connection refused")
    })
    ...
    it("should throw with wrapped message when database returns error in response", async () => {
      ...
      await expect(service.getAll("user-1")).rejects.toThrow("Erreur lors de la récupération des données.")
    })
  })

  describe("getById", () => {
    const record = { id: "42", name: "target", user_id: "user-1" }

    it("should select * with eq id and user_id and single, returning the record", async () => {
      mock.setResult(record)
      const result = await service.getById("42", "user-1")
    ...
    it("should throw on database error", async () => {
      ...
      await expect(service.getById("99", "user-1")).rejects.toThrow("item not found")
    })
    ...
    it("should throw with wrapped message when database returns error in response", async () => {
      ...
      await expect(service.getById("42", "user-1")).rejects.toThrow("Erreur lors de la récupération de l'élément.")
    })
  })
  ...
  describe("update", () => {
    ...
    const updated = { id: "1", user_id: "user-1", name: "updated" }

    it("should update with eq id and user_id, select, single and return updated record", async () => {
      mock.setResult(updated)
      const result = await service.update("1", "user-1", payload)
    ...
    it("should throw on database error", async () => {
      ...
      await expect(service.update("1", "user-1", payload)).rejects.toThrow("update failed")
    })
    ...
    it("should throw with wrapped message when database returns error in response", async () => {
      ...
      await expect(service.update("1", "user-1", { name: "test" })).rejects.toThrow("Erreur lors de la mise à jour.")
    })
  })

  describe("delete", () => {
    it("should delete with eq id and user_id", async () => {
      mock.setResult(null)
    ...
    it("should throw on database error", async () => {
      ...
      await expect(service.delete("1", "user-1")).rejects.toThrow("delete forbidden")
    })
    ...
    it("should throw with wrapped message when database returns error in response", async () => {
      ...
      await expect(service.delete("1", "user-1")).rejects.toThrow("Erreur lors de la suppression.")
    })
  })
})
